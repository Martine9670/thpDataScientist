...
puts "> Il y a #{journalists.length} handles dans cette array"

puts "\n2 - Quel est le handle le plus court de cette liste ?"
plus_court = journalists.min_by { |journalists| journalists.length }
puts ">Le handle le plus court de cette liste est : #{plus_court}"

puts "\n3 - Combien y-a-t'il de handle contenant 5 caractères (le @ ne compte pas pour un caractère)"
i = 0
five_characters = 0
while i < journalists.length
    if journalists[i].size - 1 == 5
        five_characters = five_characters + 1
    end
    i = i + 1
end
puts "> Il y a #{five_characters} handles contenant 5 caractères"
...
count = 0
while i < journalists.length
    if journalists[i][1] == journalists[i][1].upcase
        count = count + 1
    end
    i = i + 1
end
...
sorted = journalists.sort_by { |handle| handle.length }
sorted.each_with_index do |handle, index|
    puts "#{index} : #{handle}"
end

puts "\n7 - Quelle est la position dans l'array de la personne @epenser ?"
index = sorted.index("@epenser")
    if index
    puts "> @epenser est à la position #{index} dans le tableau."
    else
...
i = 0
    while i < journalists.length
    size = journalists[i].length - 1
    size_distribution[size] += 1
    i += 1
    end
    size_distribution.sort.each do |size, count|
  puts "Il y a #{count} handles avec #{size} caractères."
end
